...
if [ "$1" = "main" ]; then
    ...
    sudo docker run -v $(pwd)/data:/opt/build/data -it --cap-add=SYS_PTRACE dbi_project gdb main
elif [ "$1" = "test" ]; then
    ...
    sudo docker run -v $(pwd)/data:/opt/build/data -it --cap-add=SYS_PTRACE dbi_project gdb DBFileTest
fi