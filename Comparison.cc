...
void CNF::CreateQueryOrderMaker(OrderMaker *sortOrder,  OrderMaker *createMe) {
    ...
    for (int i = 0; i < sortOrder->numAtts; i++) {
        // search for whichAtts[i] in CNF
        if (flag) {
            for (int j = 0; j < numAnds; j++) {
                if (orLens[j] == 1) {
                    if ((orList[j][0].op == Equals) && (orList[j][0].whichAtt1 == sortOrder->whichAtts[i])) {
                        (createMe->numAtts)++;
                        createMe->whichAtts[index] = sortOrder->whichAtts[i];
                        createMe->whichTypes[index] = sortOrder->whichTypes[i];
                        index++;
                        flag = true;
                    }
                } else{
